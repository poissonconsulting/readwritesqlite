...
- preserve meta data in queries (only through `rws_read_sqlite()` and `rws_write_sqlite()` functions) 
- override the default error messages for violations of
    - CHECK constraints
    - UNIQUE constraints
    - FOREIGN KEY constraints (although foreign key constraints are enforced when writing)
