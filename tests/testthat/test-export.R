test_that("rws_export_gpkg works", {
  ...
  expect_true(all(
    c(
      "data1_geometry2.gpkg",
      "data1.gpkg",
      "data2_geometry.gpkg",
      "data2_geometry2.gpkg"
    ) %in%
      basename(files)
  ))

  expect_error(
    rws_export_gpkg(conn = conn, dir = dir),
    "File data1.gpkg already exisits. Set 'overwrite' = TRUE to overwrite."
  )

  table_expected_changes <- sf_data_deact[,
    !names(sf_data_deact) == "geometry2"
  ]
  names(table_expected_changes)[
    names(table_expected_changes) == "geometry"
  ] <- "geom"
  table_exported <- sf::st_read(
    file.path(dir, "data1_geometry2.gpkg"),
    quiet = TRUE
  )
