...
test_that("rws_rename_table informative errors", {
  ...
  rws_write(list(somedata = readwritesqlite::rws_data), exists = FALSE, conn = conn)
  expect_error(rws_drop_table("somedata2", conn),
              "^Table 'somedata2' does not exist[.]$")
  expect_error(rws_drop_table("readwritesqlite_meta", conn),
...
test_that("rws_drop_table primary key deferred", {
  ...
  expect_identical(rws_read_table("local2", conn = conn),
                   structure(list(x = c(1L, 2L, 4L), y = c(11L, 12L, 14L)), class = c("tbl_df", 
