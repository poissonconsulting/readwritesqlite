...
test_that("dbCheckTableSQLite checks reserved table names", {
  con <- DBI::dbConnect(RSQLite::SQLite(), ":memory:")
  teardown(DBI::dbDisconnect(con))

  local <- data.frame(x = as.character(1:3), select = 1:3, stringsAsFactors = FALSE)
  expect_error(dbCheckDataSQLite(con, "dbWriteSQLiteLog", local[1]),
               "'dbWriteSQLiteLog' is a reserved table")
})

test_that("dbCheckTableSQLite checks table exists", {
  con <- DBI::dbConnect(RSQLite::SQLite(), ":memory:")
  teardown(DBI::dbDisconnect(con))
...
test_that("dbCheckTableSQLite checks all columns present", {
  con <- DBI::dbConnect(RSQLite::SQLite(), ":memory:")
  teardown(DBI::dbDisconnect(con))
...
test_that("dbCheckTableSQLite corrects column order", {
  con <- DBI::dbConnect(RSQLite::SQLite(), ":memory:")
  teardown(DBI::dbDisconnect(con))
...
test_that("dbCheckTableSQLite converts factor to character", {
  con <- DBI::dbConnect(RSQLite::SQLite(), ":memory:")
  teardown(DBI::dbDisconnect(con))
