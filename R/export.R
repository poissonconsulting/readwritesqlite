...
rws_export_gpkg <- function(conn, dir, overwrite = FALSE) {
  ...
  if (inherits(meta, "try-error")) {
    err("Database must have a valid metadata table.")
  }

  init <- suppressWarnings(try(rws_read_init(conn = conn), silent = TRUE))
  if (inherits(meta, "try-error")) {
    err("Database must have a valid init table.")
  }

  meta$Geometry <- sapply(meta$MetaMeta, vld_crs, USE.NAMES = FALSE)
  if (!any(meta$Geometry)) {
    err("No geometries detected.")
  }
  ...
  for (i in seq_along(exports$ColumnMeta)) {
    ...
    for (col in names(table)) {
      if (inherits(table[, col], "difftime")) {
    ...
    if (file.exists(path) && !overwrite) {
      err(
        "File ",
        basename(path),
        " already exisits. Set 'overwrite' = TRUE to overwrite."
      )
    }

    sf::st_write(table, path, delete_dsn = overwrite, quiet = TRUE)
    ui_line(glue::glue(
      "Exported table {ui_value(tbl_name)} with spatial column {ui_value(col_name)} as {ui_value(basename(path))}"
    ))
    exported[i] <- path
  }
