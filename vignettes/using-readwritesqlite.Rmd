...
Meta data is recorded if the user uses `rws_write_sqlite()` with `meta = TRUE` (the default) to write data to an empty table.
...
If the existing factor level are already recorded in the meta data then the user can pass data with additional or rearranged levels without deleting the existing data.

### Duplicate Data

When passing data frames to `rws_write_sqlite()` in the form of an environment or named list, each table must be represented by just one data frame if `unique = TRUE` (the default).
...
The table names can of course be manipulated and `list2env()` to assigns the data frames to the current environment.
```{r, eval=FALSE}
