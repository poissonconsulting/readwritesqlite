...
have already been created by the database designer (presumably with the necessary checks and foreign keys).
...
not_a_table <- 1
rws_write_sqlite(environment())
rws_list_tables()
```
Objects which are not data frames are silently filtered from enviroments but cause an error with lists.
